...
import { Loader2, LogOut, Eye, EyeOff, Bell, BellOff, UserPen, Moon, Sun, ChevronDown, ChevronUp, Save, Globe, Shield, ClipboardList } from "lucide-react";
...
export function SettingsPage({ profile, onLogout, onNavigate, lang, onLangChange }: SettingsPageProps) {
  const { t } = useLang();
    ...
    async function checkPerms() {
      if (Capacitor.isNativePlatform()) {
        try {
          ...
          setNotifPerm(display === 'granted' ? 'granted' : display === 'denied' ? 'denied' : 'default');
        } catch (e) {
  ...
  async function requestNotifPermission() {
    ...
    if (granted) {
      ...
      saveNotif(true, notifTime);
      await notificationService.sendTestNotificationNow();
    } else {
      setNotifPerm("denied");
      setNotifEnabled(false);
      alert("Bildirishnomalarga ruxsat berilmadi. Iltimos, sozlamalardan ruxsat bering.");
    }
  }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="premium-card p-4 mb-3">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-xp/10 text-xp">
            {isDark ? <Moon size={16} /> : <Sun size={16} />}
          </div>
          <div>
            <p className="text-sm font-semibold">{t('settings_theme' as any) || 'Ilova ko\'rinishi'}</p>
            <p className="text-xs text-muted-foreground">{t('settings_theme_sub' as any) || 'Mavzuni tanlang'}</p>
          </div>
        </div>
          ...
          <button type="button"
            onClick={async () => {
              if (notifPerm !== "granted") {
                const ok = await notificationService.initNotifications();
                if (ok) setNotifPerm("granted");
                else { setNotifPerm("denied"); return; }
              }
              const next = !notifEnabled;
              setNotifEnabled(next);
              saveNotif(next, notifTime);
            }}
        ...
        {notifEnabled && notifPerm === "granted" && (
          <div className="mt-3 space-y-3">
            ...
            <button
              ...
              style={{ background: isDark ? "rgba(255,255,255,0.05)" : "#F3F4F6", color: "var(--foreground)", border: `1px solid ${isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"}` }}
            >
              🚀 Sinab ko'rish
            </button>
          </div>
