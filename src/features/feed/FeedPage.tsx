...
import { Loader2, Rss, UserPlus, Flame, Lock, Sparkles, Check, ClipboardList, Zap } from "lucide-react";
  ...
  function ReactionBar({ itemId }: { itemId: string }) {
    ...
    return (
      <div className="flex items-center gap-1.5 mt-2">
        ...
        >
          <Flame size={12} className={rx.myFire ? "fill-orange-500" : ""} /> {rx.fire > 0 && <span>{rx.fire}</span>}
        </button>
        <button
          type="button"
        ...
        >
          <Sparkles size={12} className={rx.myClap ? "fill-blue-400" : ""} /> {rx.clap > 0 && <span>{rx.clap}</span>}
        </button>
      </div>
    );
  }
    ...
    <div className="max-w-2xl flex flex-col gap-4">
      ...
      {loading ? (
        ...
        <div style={card} className="p-10 flex flex-col items-center gap-3">
          <Rss size={48} className="text-slate-400 mb-2 opacity-20" />
